...
const PrismaClientRuntime = pkg.PrismaClient;
...
import { generateAccessToken, generateRefreshToken, verifyRefreshToken, hashPassword, comparePassword } from '../services/auth.service.js';
...
router.post('/login', async (req, res, next) => {
    try {
        ...
        logger.error('Login master error', { error: error instanceof Error ? error.message : 'Unknown', stack: error?.stack });
        next(error);
    }
});
...
router.post('/login-tenant', async (req, res, next) => {
    try {
        ...
        logger.error('Login tenant error', { error: error instanceof Error ? error.message : 'Unknown', stack: error?.stack });
        next(error);
    }
});
    ...
    try {
        ...
        const responseData = {
            ...
            tenant: reqTenant ? {
                ...
                name: reqTenant.name,
                cnpj: reqTenant.cnpj,
                modulesEnabled: reqTenant.modulesEnabled || []
            } : null
        };
