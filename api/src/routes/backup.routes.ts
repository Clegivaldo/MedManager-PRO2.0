...
router.post('/db/:tenantId', authenticateToken, requirePermissions([PERMISSIONS.BACKUP_CREATE]), async (req, res, next) => {
  try {
    ...
    pg.on('close', async (code) => {
      ...
      logger.info('Backup completed', { tenantId, file: fsSync.existsSync(zipFile) ? zipFile : sqlFile });
      return res.json({ success: true, message: 'Backup completed', file: fsSync.existsSync(zipFile) ? zipFile : sqlFile });
    });
