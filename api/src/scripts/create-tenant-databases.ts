  ...
  try {
    ...
    for (const tenant of tenants) {
      try {
        ...
        // Testar conexão com o banco do tenant
        await tenantPrisma.$connect();
        await tenantPrisma.$disconnect();

        logger.info(`Database already exists for tenant: ${tenant.name}`);
      } catch (error) {
        // Se não existe, criar o banco de dados
        logger.info(`Creating database for tenant: ${tenant.name}`);
